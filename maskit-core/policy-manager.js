...
class PolicyManager {
  constructor(policyPath, options = {}) {
    ...
    this.currentPolicy = SECURE_DEFAULT;
    this.lastLoad = 0;
  ...
  validate(policy) {
    if (!policy || typeof policy !== 'object' || typeof policy.version !== 'string') return { ok: false, error: 'Policy version is required' };
    if (!MODES.has(policy.mode)) return { ok: false, error: 'Invalid policy mode' };
    if (!policy.actions || typeof policy.actions !== 'object' || Array.isArray(policy.actions)) return { ok: false, error: 'Policy actions are required' };
    for (const [rule, action] of Object.entries(policy.actions)) if (!/^[A-Za-z0-9_.-]+$/.test(rule) || !ACTIONS.has(action)) return { ok: false, error: `Invalid action for ${rule}` };
    return { ok: true };
  }
  reload() {
    try {
      let next = SECURE_DEFAULT;
      if (fs.existsSync(this.policyPath)) {
        const parsed = JSON.parse(fs.readFileSync(this.policyPath, 'utf8'));
        const validation = this.validate(parsed);
        if (!validation.ok) throw new Error(validation.error);
        next = Object.freeze({ version: parsed.version, mode: parsed.mode, actions: { ...parsed.actions } });
      }
      ...
      this.currentPolicy = SECURE_DEFAULT;
      this.lastLoad = Date.now();
      this.onAudit({ type: 'policy_fallback', reason: 'invalid_policy', policyVersion: SECURE_DEFAULT.version });
      this.notifySubscribers();
      return false;
    }
  }
  startWatching() {
    ...
    this.watcher = fs.watch(dir, (eventType, filename) => {
      if (filename && filename.toString() === path.basename(this.policyPath)) {

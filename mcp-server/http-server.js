...
function checkRateLimit(token) { const now = Date.now(); if (!requestCounts.has(token)) requestCounts.set(token, { count: 0, resetAt: now + RATE_LIMIT_WINDOW }); const entry = requestCounts.get(token); if (now > entry.resetAt) { entry.count = 0; entry.resetAt = now + RATE_LIMIT_WINDOW; } if (entry.count >= RATE_LIMIT_MAX_REQUESTS) return false; entry.count++; return true; }
...
function parseBody(req) { return new Promise((resolve, reject) => { let body = ""; let size = 0; req.on("data", (chunk) => { size += chunk.length; if (size > MAX_BODY_SIZE) { req.destroy(); reject(new Error("Request body too large (max 1MB)")); return; } body += chunk; }); req.on("end", () => { try { resolve(JSON.parse(body)); } catch (err) { reject(new Error("Invalid JSON: " + err.message)); } }); req.on("error", (err) => reject(new Error("Request error: " + err.message))); }); }
...
function isAuthenticated(req) {
    ...
    return candidate.length === expected.length && crypto.timingSafeEqual(candidate, expected); // FIXED: 8 — compare secret material in constant time
}
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (req.method === "GET" && pathname === "/rules") { const settings = config.readConfig(); return sendJson(res, engine.getRules(settings)); }
        if (req.method === "POST" && pathname === "/scan") { let body; try { body = await parseBody(req); } catch (err) { return sendJson(res, { error: err.message }, 400); } if (!body || typeof body.text !== "string") return sendJson(res, { error: "Missing or invalid 'text' field" }, 400); return sendJson(res, engine.scanText(body.text, config.getSettingsForApp(body.app))); }
        ...
        if (req.method === "POST" && pathname === "/policy") { let body; try { body = await parseBody(req); } catch (err) { return sendJson(res, { error: err.message }, 400); } if (!body || typeof body.text !== "string") return sendJson(res, { error: "Missing or invalid 'text' field" }, 400); return sendJson(res, engine.evaluatePolicy(body.text, config.getSettingsForApp(body.app))); }
        res.writeHead(404, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "Not found" }));
    } catch (err) { sendJson(res, { error: err.message }, 500); }
});
server.listen(PORT, "127.0.0.1", () => { console.log("Maskit HTTP server listening on http://127.0.0.1:" + PORT); console.log("API token configured (use status endpoint to verify)"); });

...
function compact(result) { if (!result.findings.length) { console.log("No sensitive data found."); return; } console.log(`Found ${result.findings.length} sensitive item(s):`); result.findings.forEach((f) => console.log(`  - ${f.type} [${f.severity}]: ${f.value.slice(0, 20)}${f.value.length > 20 ? "..." : ""}`)); console.log(`Risk score: ${result.riskScore} (${result.riskLevel})`); }
...
switch (command) {
  ...
  case "report": { if (positional[0] !== "generate") { usage(); process.exitCode = 1; break; } const daysIndex = args.indexOf("--days"); const outputIndex = args.indexOf("--output"); const reportData = report.generate({ days: daysIndex >= 0 ? args[daysIndex + 1] : 30 }); const outputDir = report.writeReport(reportData, outputIndex >= 0 ? args[outputIndex + 1] : path.join(process.cwd(), "reports")); if (json) print({ outputDir, report: reportData }); else console.log(`AI Security Report generated locally in ${outputDir}`); break; }

...
const { recordTool } = require("./inventory");
...
function compact(result) { if (!result.findings.length) return console.log("No sensitive data found."); console.log(`Found ${result.findings.length} sensitive item(s):`); result.findings.forEach(f => console.log(`  - ${f.type} [${f.severity}]: ${f.value.slice(0, 20)}${f.value.length > 20 ? "..." : ""}`)); console.log(`Risk score: ${result.riskScore} (${result.riskLevel})`); }
...
function usage() { console.log("Maskit CLI\n  scan <text> [--json]\n  scan-file <path> [--json] [--exit-on-risk]\n  redact <text> [--json]\n  risk <text> [--json]\n  report generate [--days 30] [--output reports] [--json]\n  inventory add <tool> <surface> [approved|unknown|blocked]\n  simulate <text> [--json]"); }
...
case "report": { if (positional[0] !== "generate") { usage(); process.exitCode = 1; break; } const daysIndex = args.indexOf("--days"); const outputIndex = args.indexOf("--output"); const data = report.generate({ days: daysIndex >= 0 ? args[daysIndex + 1] : 30 }); const outputDir = report.writeReport(data, outputIndex >= 0 ? args[outputIndex + 1] : path.join(process.cwd(), "reports")); if (json) print({ outputDir, report: data }); else console.log(`AI Security Report generated locally in ${outputDir}`); break; }
case "inventory": { if (positional[0] !== "add" || !positional[1] || !positional[2]) { usage(); process.exitCode = 1; break; } const file = recordTool({ tool: positional[1], surface: positional[2], status: positional[3] || "unknown" }, config); if (json) print({ inventoryFile: file }); else console.log(`AI inventory updated locally: ${file}`); break; }
case "simulate": { requireInput(); const result = simulatePolicy(engine, input, { ...config.readConfig(), _context: { source: "cli", app: "simulation" } }); if (json) print(result); else { console.log(`Simulation only: audit mutated = ${result.mutated}`); console.log(`Risk: ${result.riskScore} (${result.riskLevel})`); result.explanations.forEach(item => console.log(`${item.type}: ${item.explanation} Confidence ${item.confidence}`)); console.log(result.redactedText); } break; }

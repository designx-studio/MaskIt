...
function assertFinding(text, ruleName, severity) { const finding = findingFor(text, ruleName); assert.ok(finding, `${ruleName} was not detected`); assert.strictEqual(finding.ruleName, ruleName); assert.ok(typeof finding.confidence === 'number' && finding.confidence > 0 && finding.confidence <= 1); if (severity) assert.strictEqual(finding.severity, severity); }
...
for (const fixture of ['ordinary prose with no credentials', 'const test = "test-id-1234567890";', 'random identifier 7f3a9c1e-12ab-4f3d-9d10-123456789abc']) { const result = engine.scanText(fixture); assert.strictEqual(result.allFindings.length, 0, `false positive detected: ${fixture}`); }

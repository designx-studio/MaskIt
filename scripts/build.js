...
const extensionFiles = ["manifest.json", "settings.js", "detector.js", "sanitizer.js", "editors.js", "content.js", "background.js", "ai-intercept.js", "popup.html", "popup.css", "popup.js", "options.html", "options.css", "options.js", "privacy-policy.html", "icons/icon16.png", "icons/icon48.png", "icons/icon128.png"];
...
function copyDir(relDir, destDir) { const src = path.join(root, relDir), dest = path.join(destDir, relDir); if (!fs.existsSync(src)) throw new Error(`Missing required directory: ${relDir}`); fs.mkdirSync(dest, { recursive: true }); fs.cpSync(src, dest, { recursive: true }); }
function zipDir(source, outZip) { execSync(`cd "${source}" && zip -qr "${outZip}" .`, { stdio: "inherit" }); }
...
function buildMcp() { const staging = path.join(distDir, "maskit-mcp"); const out = path.join(distDir, "maskit-mcp.tar.gz"); rmrf(staging); fs.mkdirSync(staging, { recursive: true }); copyDir("mcp-server", staging); copyDir("engine", staging); copyFile("CORE_CONTRACT.md", staging); execSync(`tar -czf "${out}" -C "${distDir}" maskit-mcp`, { stdio: "inherit" }); rmrf(staging); }
rmrf(distDir); fs.mkdirSync(distDir, { recursive: true });
buildExtension("chrome");
buildExtension("edge");
buildExtension("opera");
buildExtension("firefox", (manifest) => { manifest.browser_specific_settings = { gecko: { id: "maskit@maskit.local", strict_min_version: "109.0" } }; });
buildExtension("extension");
buildMcp();
console.log("Built Chrome, Edge, Opera, Firefox, legacy extension, and MCP artifacts.");

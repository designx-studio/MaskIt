...
function withManagedPolicy(scanSettings) {
  const merged = { ...(scanSettings || {}) };
  if (!merged._policy) merged._policy = policyManager.getPolicy();
  return merged;
}

function scanText(text, scanSettings) {
  const managedSettings = withManagedPolicy(scanSettings);
  const result = detector.scanText(text, managedSettings);
  const events = (result.events || []).map((event, index) => {
    ...
    return context.createEventFromFinding({ finding: decision?.finding || { type: event.dataType || 'unknown', value: '' }, decision: decision || { action: 'redact' }, context: (managedSettings && managedSettings._context) || {}, riskScore: result.riskScore, settings: managedSettings });
  });
...
function evaluatePolicy(text, scanSettings) {
  ...
  return { allowed: !result.policyDecisions.some((d) => d.action === 'block'), findings: result.findings, allFindings: result.allFindings, policyDecisions: result.policyDecisions, riskScore: result.riskScore, riskLevel: result.riskLevel, blocked: result.policyDecisions.filter((d) => d.action === 'block').length, allowedByPolicy: result.policyDecisions.filter((d) => d.action === 'allow').length, redacted: result.policyDecisions.filter((d) => d.action === 'redact').length, reason: result.policyDecisions.length ? `Found ${result.allFindings.length} sensitive item(s)` : 'No sensitive data detected' };
}

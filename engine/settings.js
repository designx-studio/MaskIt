...
const MASKIT_DEFAULTS = {
    ...
    siteListMode: "all", siteList: [], EMAIL: true, PHONE: true, CARD: true,
    MPESA: true, SSN: true, API_KEY: true, IP_ADDRESS: true, PASSPORT: true,
    BANK_ACCOUNT: true, customRules: [], pauseAutoResume: true,
    pauseAutoResumeSecs: 30, browserAIProtection: true
};
...
function createAuditEvent({ type, severity, source, app, action, riskScore, matchedRule, policyApplied, value, saltProvider, chainHash }) {
    return { id: "evt_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8), timestamp: Date.now(), type: String(type || "UNKNOWN"), severity: String(severity || "medium"), source: String(source || "unknown"), app: String(app || "unknown"), action: String(action || "redacted"), riskScore: Number(riskScore) || 0, matchedRule: String(matchedRule || ""), policyApplied: String(policyApplied || "default"), unmaskToken: value ? hashSensitive(value, saltProvider) : null, unmaskedAt: null, unmaskedDuration: null, chainHash: chainHash || null };
}
function getHashSalt(saltProvider) {
    if (_hashSalt) return _hashSalt;
    if (saltProvider && typeof saltProvider.getHashSalt === "function") _hashSalt = saltProvider.getHashSalt();
    if (!_hashSalt) _hashSalt = crypto.randomBytes(32).toString("hex");
    if (saltProvider && typeof saltProvider.setHashSalt === "function") saltProvider.setHashSalt(_hashSalt);
    return _hashSalt;
}
...
function pruneAuditLog(events, retentionDays) { const cutoff = Date.now() - (retentionDays || 30) * 86400000; return (events || []).filter((e) => e.timestamp >= cutoff); }
...
function parseTime(value) { if (typeof value !== "string") return null; const [h, m] = value.split(":").map(Number); return Number.isInteger(h) && Number.isInteger(m) && h >= 0 && h < 24 && m >= 0 && m < 60 ? h * 60 + m : null; }
...
function isRegexSafe(pattern) { const text = String(pattern || "").trim(); return !!text && text.length <= REGEX_LIMITS.maxPatternLength && !(/\{(\d+)?,\}/.test(text) || /(\([^)]*[+*][^)]*\))[+*{]/.test(text) || /(\.\*)[+*]|\(\.\*\)[+*]/.test(text) || /\([^)]+\)\{(\d+),\}/.test(text)); }
...
module.exports = { MASKIT_SUPPORTED_SITES, REGEX_LIMITS, MASKIT_DEFAULTS, SEVERITY_DEFAULTS, SEVERITY_WEIGHTS, SEVERITY_LEVELS, MASKIT_STATS_DEFAULTS, AUDIT_LOG_DEFAULTS, createAuditEvent, hashSensitive, hashSensitiveAsync, getHashSalt, pruneAuditLog, POLICY_ACTIONS, MASKIT_POLICY_DEFAULTS, selectPolicy, getPolicyAction, KILLSWITCH_DEFAULTS, isAIToolsAllowed, parseTime, getUserIdentity, isUserExempt, hostnameMatches, isSiteAllowed, maskPreview, escapeHtml, isRegexSafe, validateCustomRegexPattern, limitScanText };

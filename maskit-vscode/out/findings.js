...
exports.scanCurrentFile = scanCurrentFile;
exports.scanSelection = scanSelection;
exports.showFindings = showFindings;
...
function scanCurrentFile() {
    ...
    const findings = (0, diagnostics_1.triggerScan)(editor.document);
    if (findings.length === 0) {
        vscode.window.showInformationMessage("MaskIt: No sensitive credentials detected in this file.");
    }
    else {
        vscode.window.showWarningMessage(`MaskIt: Detected ${findings.length} sensitive credential(s) in this file.`);
    }
}
function scanSelection() {
    ...
    const result = (0, scanner_1.scanText)(text, editor.document.languageId);
    if (result.findings.length === 0) {
        ...
        vscode.window.showWarningMessage(`MaskIt Warning: Sensitive data found in selection: ${list}`);
    }
}
function showFindings() {
    ...
    const diagnostics = (0, diagnostics_1.getActiveDiagnostics)(editor.document);
...
function explainFindingDetails(diagnostic) {
    ...
    vscode.window.showInformationMessage(`MaskIt Explanation [${type}]:\n\n` +
        `Risk:\nCritical\n\n` +
        `Reason:\n${reason}\n\n` +
        `Action:\n${recommendation}`, { modal: true });
}
